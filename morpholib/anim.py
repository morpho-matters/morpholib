...
class Frame(BoundingBoxFigure):
    ...
    #
    # Optionally, instead of a permutation, a `swap` dict can be
    # inputted by keyword like so:
    #   frame.permute(swap={...})
    # where `swap` is a dict mapping indices to indices where the
    # subfigures corresponding to the item-pairs will have their
    # index positions swapped.
    def permute(self, *permutation, swap=None):
        if swap is not None:
            if len(permutation) > 0:
                raise TypeError("Cannot receive both a permutation and a swap dict.")
            # Convert swap dict into a permutation dict
            permutation = dict()
    ...
    # Returns the index position of a given subfigure or subfigure name
    # in the `figures` list.
    def index(self, subfig):
        if isinstance(subfig, morpho.Figure):
            return self.figures.index(subfig)
        elif isinstance(subfig, str):
            return self._names[subfig]
        raise TypeError("`subfig` must be Figure or str type.")
...
class Animation(object):
    ...
    # Returns the width/height aspect ratio of the animation's
    # window shape.
    def aspectRatioWH(self):
